...
pywebio.config(title="霍兰德职业性格测试平台-志愿智能")  # 标题
...
def base():
    ...
    toast("请完成下面各题共90题，符合请选择 认同 否则为 不认同", 2, "center", "success")  # 通知消息
    order = []
    question = []
    choice = []
    types = []
    with open(
        "server/Holland/Holland_testQuestions.txt", "r", encoding="utf-8"
    ) as file:
        for lines in file:
            line = lines.replace("\n", "")
            ord = line.split("．")[0]
    ...
    data = pd.DataFrame(
        {"order": order, "question": question, "types": types, "choice": choice}
    )
    ...
    result = []
    with open("server/Holland/Holland_result.txt", "r", encoding="utf-8") as file:
        for lines in file:
            result.append(lines)
...
def drawing(countYes, countYesList, maxType, result):
    radar = Radar({"theme": ThemeType.LIGHT})
    radar.add_schema(
        schema=[
            opts.RadarIndicatorItem(
                name="{}".format(countYes.index.tolist()[0]), max_=15
            ),
            opts.RadarIndicatorItem(
                name="{}".format(countYes.index.tolist()[1]), max_=15
            ),
            opts.RadarIndicatorItem(
                name="{}".format(countYes.index.tolist()[2]), max_=15
            ),
            opts.RadarIndicatorItem(
                name="{}".format(countYes.index.tolist()[3]), max_=15
            ),
            opts.RadarIndicatorItem(
                name="{}".format(countYes.index.tolist()[4]), max_=15
            ),
            opts.RadarIndicatorItem(
                name="{}".format(countYes.index.tolist()[5]), max_=15
            ),
        ]
    )
    radar.add(
    ...
    radar.set_global_opts(
        title_opts=opts.TitleOpts(title="霍兰德职业性格测试结果"),
...
class run:
    def __init__(self):
        put_html('<link rel="stylesheet" href="css/HollandTest.css">')
