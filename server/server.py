...
redis_host = os.getenv("REDIS_HOST", "localhost")  # 从环境变量中读取Redis地址，默认为localhost
...
def login():
    ...
    return render_template("check_ScreenSize.html")
...
def getSchoolList():
    ...
    if type != "全部":
        ...
        if type == "教育部":
            ...
            sql += (
                f"governing_department != '教育部' AND governing_department != '中央军委训练管理部'"
            )
        elif type == "军校":
...
def hotSchool():
    ...
    hotSchoolMAXCount = (
        DB.get(f"SELECT COUNT(*) AS result_count FROM colleges WHERE {sql_snippet};")[0][0] - 10)  # fmt: skip
    hotSchoolCount = random.randint(0, hotSchoolMAXCount)
    result = DB.get(
        f"SELECT schoolName,schoolBadge,schoolID, REPLACE(university_type, '院校', '类') AS university_type,education_level FROM colleges WHERE {sql_snippet} ORDER BY followCount DESC LIMIT {hotSchoolCount},10;"
    )
