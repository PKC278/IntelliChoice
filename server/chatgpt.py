...
from getFromDB import get_from_db
import requests
import time
...
class ChatGPT:
    ...
    def run_conversation(user_question, history):
        ...
        try:
            location = requests.post("http://localhost:8080/api/userLocation").json()[
                "location"
            ]
        except:
            location = "未知"
        ...
        messages.append(
            {
                "role": "system",
                "content": f"现在是北京时间:{beijing_time},用户IP归属地:{location},你是志愿智能平台的AI,我想让你成为一位高考咨询专家，使用markdown语法回复。",
            }
        )
        ...
        # 步骤2：检查GPT是否想调用一个函数
        if first_chunk.get("function_call"):
            arguments = ""
            ...
            while attempt_count < 3:
                try:
                    second_response = openai.ChatCompletion.create(
                        model="gpt-3.5-turbo-0613", messages=messages, stream=True
                    )  # 从GPT中获取一个新的响应
                    break
                except Exception as e:
                    attempt_count += 1
    ...
    def generate_title(history):
        ...
        for group in history:
            ...
            for key, value in group.items():
                messages.append({"role": key, "content": value})
        messages.append(
            {"role": "user", "content": "根据对话内容生成一个会话标题,不要带任何标点符号,直接输出一个短标题"}
        )
